...
@protocol NewSimplePingDelegate;

/*! Controls the IP address version used by NewSimplePing instances.
 */

typedef NS_ENUM(NSInteger, NewSimplePingAddressStyle) {
    NewSimplePingAddressStyleAny,          ///< Use the first IPv4 or IPv6 address found; the default.
    NewSimplePingAddressStyleICMPv4,       ///< Use the first IPv4 address found.
    NewSimplePingAddressStyleICMPv6        ///< Use the first IPv6 address found.
};
...
@interface NewSimplePing : NSObject
...
@property (nonatomic, weak, readwrite, nullable) id<NewSimplePingDelegate> delegate;

/*! Controls the IP address version used by the object.
 *  \details You should set this value before starting the object.
 */

@property (nonatomic, assign, readwrite) NewSimplePingAddressStyle addressStyle;
...
@protocol NewSimplePingDelegate <NSObject>
...
- (void)newSimplePing:(NewSimplePing *)pinger didStartWithAddress:(NSData *)address;
...
- (void)newSimplePing:(NewSimplePing *)pinger didFailWithError:(NSError *)error;
...
- (void)newSimplePing:(NewSimplePing *)pinger didSendPacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber;
...
- (void)newSimplePing:(NewSimplePing *)pinger didFailToSendPacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber error:(NSError *)error;
...
- (void)newSimplePing:(NewSimplePing *)pinger didReceivePingResponsePacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber;
...
- (void)newSimplePing:(NewSimplePing *)pinger didReceiveUnexpectedPacket:(NSData *)packet;
...
typedef struct NICMPHeader NICMPHeader;

__Check_Compile_Time(sizeof(NICMPHeader) == 8);
__Check_Compile_Time(offsetof(NICMPHeader, type) == 0);
__Check_Compile_Time(offsetof(NICMPHeader, code) == 1);
__Check_Compile_Time(offsetof(NICMPHeader, checksum) == 2);
__Check_Compile_Time(offsetof(NICMPHeader, identifier) == 4);
__Check_Compile_Time(offsetof(NICMPHeader, sequenceNumber) == 6);
